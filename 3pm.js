  ...
  query GetAccounts($first: Int!, $after: String, $toursAfter: String, $showsAfter: String, $itemizedTransactionsAfter: String) {
    organization {
      accounts(first: $first, after: $after) {
        ...
        nodes {
          name
          tours(first: 10, after: $toursAfter) {
            pageInfo {
              endCursor
              hasNextPage
            }
            nodes {
              name
              shows(first: 10, after: $showsAfter) {
                pageInfo {
                  endCursor
                  hasNextPage
                }
                nodes {
                  ...
                  itemizedTransactions(first: 10, after: $itemizedTransactionsAfter) {
                    pageInfo {
                      endCursor
                      hasNextPage
                    }
          ...
          for (const tour of fetchedTours) {
            ...
            while (showsHasNextPage) {
              ...
              for (const show of fetchedShows) {
                ...
                while (itemizedTransactionsHasNextPage) {
                  variables.itemizedTransactionsAfter =
...
const exportShowsToCSV = (shows) => {
  ...
  try {
    ...
    fs.writeFileSync("ShowsFrom2022 - 3pm.csv", csvString);
