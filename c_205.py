...
QUERY = '''
query GetAllShowsAndTransactions($after: String) {
  organization {
    accounts(first: 10) {
      nodes {
        name
          ...
          nodes {
            ...
            shows(first: 10, after: $after) {
              nodes {
                ...
                itemizedTransactions(first: 100) {
                  nodes {
                    artistName
...
def fetch_all_data():
    ...
    after = None
    while True:
        accounts = fetch_data(after)
        all_accounts.extend(accounts)
        if not accounts or not accounts[-1]["tours"]["nodes"][-1]["shows"]["pageInfo"]["hasNextPage"]:
            break
        after = accounts[-1]["tours"]["nodes"][-1]["shows"]["pageInfo"]["endCursor"]
    return all_accounts
...
def flatten_show_data(account):
    ...
    for tour in account.get("tours", {}).get("nodes", []):
        for show in tour.get("shows", {}).get("nodes", []):
            if not (datetime(2023, 1, 1) <= datetime.strptime(show["showDate"], "%Y-%m-%d") <= datetime(2023, 6, 30)):
                continue
...
if __name__ == "__main__":
    filename = "shows_data_first_half_2023.csv"
    
    accounts = fetch_all_data()
    flattened_data = []
    for account in accounts:
        flattened_data.extend(flatten_show_data(account))
    export_to_csv(flattened_data, filename)